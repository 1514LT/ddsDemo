...
void LeaderListener::on_publication_matched(DataWriter * dataWriter, const PublicationMatchedStatus & info)
{
...
bool Leader::initPubType(const std::string & topicName, const std::string & typeName, TopicDataType * dataType, DataWriterListener * listener)
{
...
bool Leader::init(int transportType)
{
  ...
  m_writers.reserve(1);
  return 
  initPubType("RestartInfoTopic","RestartInfo",new  RestartInfoPubSubType,&m_listener);
}

bool Leader::RestartInfoMatched()
{
  return m_listener.m_matched > 0;
}
bool Leader::PublishRestartInfo(RestartInfo &restartInfo)
{
  return m_writers.size() > 0 && m_writers[0].second->write(&restartInfo);
}