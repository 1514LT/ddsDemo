...
SlavePubListener::SlavePubListener(/* args */)
:m_matched(0)
{
}

SlavePubListener::~SlavePubListener()
{
}

void SlavePubListener::on_publication_matched(DataWriter * dataWriter, const PublicationMatchedStatus & info)
{
...
bool SlavePublisher::initPubType(const std::string & topicName, const std::string & typeName, TopicDataType * dataType, DataWriterListener * listener)
{
...
bool SlavePublisher::init(int transportType)
{
  ...
  paritcipantQos = m_transport.getUdpQos();
    break;
  case TCPTRANSPORT:
  m_transport.initTcpQos(TCPCLIENT,8080);
...
bool SlavePublisher::replayInfoMatched()
{
  return m_listener.m_matched > 0;
}
bool SlavePublisher::publishReplyInfo(ReplyInfo &replyInfo)
{
  return m_writers.size() > 0 && m_writers[0].second->write(&replyInfo);
}